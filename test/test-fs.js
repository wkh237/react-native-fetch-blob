...
describe('write file range', (report, done) => {

  function testOfLength(base, len) {
    let dest = dirs.DocumentDir + '/write-range' + Date.now() + '-' + len
    let data = []
    for(var i = 0; i < base + Math.random() *len; i++) {
      data.push(0)
    }
    // prepare test data
    fs.writeFile(dest, 'ascii', data).then(() => {
...
describe('read file range', (report, done) => {
